...
// variable MQX_WORK_DIR. When not set, it uses the directory where the application's
// binary is located.
func WorkDir() string {
	workDirOnce.Do(func() {
		workDir = os.Getenv("MQX_WORK_DIR")
		if workDir != "" {
			return
		}
...
// It reads the value of environment variable MQX_CUSTOM. When not set, it uses the work
// directory returned by WorkDir function.
func CustomDir() string {
	customDirOnce.Do(func() {
		customDir = os.Getenv("MQX_CUSTOM")
		if customDir != "" {
			return
		}
